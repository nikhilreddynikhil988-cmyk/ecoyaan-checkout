        ...
        <div className="flex justify-between">
          {steps.map((step) => (
            <div key={step.href} className="flex flex-col items-center">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step.step <= currentStep ? "bg-white text-green-600" : "bg-green-700 text-white"
                  }`}
              >
                {step.step}
              </div>
              <Link
                href={step.href}
                className={`mt-2 text-sm hover:underline ${pathname === step.href ? "font-bold underline" : ""
                  }`}
              >
                {step.label}
              </Link>
